...
export default function AdminApplicationsPage() {
  ...
  return (
    <>
      <div className="section-title">All Applications ({apps.length})</div>
      {loading
        ? <div style={{ textAlign: 'center', padding: 40 }}><span className="spinner spinner-blue" style={{ width: 28, height: 28 }} /></div>
        : !apps.length
          ? <EmptyState title="No applications yet" sub="Job seeker applications will appear here." />
          : apps.map(app => (
              <div key={app.id} className="panel" style={{ marginBottom: 14 }}>
                ...
                <div style={{ fontSize: 13, color: 'var(--ink-soft)', margin: '8px 0 4px' }}>
                  ...
                  {' · '}
                  Target Employer:{' '}
                  <Link href={`/admin/employers/${app.employerId}`} style={{ color: 'var(--blue)', fontWeight: 600, textDecoration: 'none' }}>
                    View Employer ↗
                  </Link>
                </div>
